...
my @bad_files = qw(
    valgrind.out stdout.txt stderr.txt
    tests/01-hello tests/02-array-sum tests/03-list-sum
    tests/04-two-lists tests/05-many-sizes
);

my $bads = 0;
for my $path (@bad_files) {
    $bads++ if (-e $path);
}
...
ok($stats->{freelen} == 1,  "04 freelist");
...
ok($stats->{freelen} >= 1 && $stats->{freelen} <= 100,  "05 freelist");
